  ...
  return (
    <div className="w-full py-8 sm:py-14 overflow-hidden relative z-10 flex flex-col gap-4 sm:gap-6">
      <div className="w-full text-center px-4">
        <p className="text-xs sm:text-sm font-semibold uppercase tracking-widest text-black/60">Core Arsenal</p>
      </div>
      ...
      >
        ...
        >
          {marqueeItems.map((tech, i) => (
            <div 
              key={i} 
              className="px-5 sm:px-7 py-2.5 sm:py-3.5 rounded-full border border-black/5 bg-white/50 backdrop-blur-xl shadow-sm text-black/80 font-medium text-sm sm:text-base flex items-center gap-2.5 sm:gap-3 hover:bg-white/80 hover:scale-105 transition-all duration-300 cursor-default shrink-0"
            >
