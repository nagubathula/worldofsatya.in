    ...
    >
      <div className="py-16 sm:py-32 px-4 sm:px-8 max-w-3xl mx-auto w-full flex flex-col gap-8 sm:gap-10" suppressHydrationWarning>
        <motion.div variants={itemAnim} className="mb-2 sm:mb-10" suppressHydrationWarning>
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-black/5 text-black/60 text-xs font-medium mb-4 uppercase tracking-widest" suppressHydrationWarning>
          ...
          {(limit ? achievements.slice(0, limit) : achievements).map((item, i) => {
            ...
            const CardContent = (
              <motion.div
                ...
                className={`group flex flex-col p-5 sm:p-8 bg-white rounded-2xl sm:rounded-3xl border border-black/[0.04] shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] transition-all duration-300 ${isLink ? 'cursor-pointer block' : ''}`}
              >
                <div className="flex justify-between items-start mb-3 sm:mb-4" suppressHydrationWarning>
                  ...
                  {isLink && (
                    <ArrowUpRight size={18} className="text-black/40 group-hover:text-black transition-colors" />
                  )}
                </div>
                <h3 className="text-lg sm:text-xl font-semibold text-black tracking-tight mb-1.5 sm:mb-2">{item.title}</h3>
                <p className="text-black/60 text-xs sm:text-sm leading-relaxed font-medium">{item.description}</p>
              </motion.div>
            );
        ...
        {limit && achievements.length > limit && (
          <motion.div variants={itemAnim} className="mt-4 sm:mt-8 flex justify-center" suppressHydrationWarning>
            <Link href="/achievements" className="px-6 py-3 bg-black text-white rounded-full text-sm font-medium hover:bg-black/90 transition-colors">
